...
export function ColourQuestion({ initial, onAnswer, onBack }: ColourQuestionProps): JSX.Element {
  ...
  return (
    <Question
      ...
      // nothing about the *shape* of an answer can take it away. Before #142, a swatch plus junk
      // in the box killed the button even though a perfectly good answer was selected.
      //
      // **And typing into the box counts as having tried** (CL-1, finding A-1). With nothing
      // picked and `zzzzzz` in the box, `answer` was still `""`, so `Continue` went away — **and
      // a disabled button leaves the tab order**, so a keyboard owner tabbed the whole step,
      // wrapped, and never met the button, the sentence, or any cue that something was wrong.
      //
      // #368 made that every screen's rule and this screen stopped being special: the button is
      // never disabled anywhere. Nothing picked and nothing typed is answered with this sentence;
      // junk in the box is answered with the field's own, below; the swatch stands under either.
      unanswered={
        answer === "" && typed.trim() === ""
